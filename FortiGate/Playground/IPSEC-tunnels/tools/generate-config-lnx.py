...
index_end = 4000  # Count needs to be less than the IP addresses available in the IP network
...
"""
template_exabgpcli = """echo env exabgp.api.pipename=exabgp{{ index }} exabgpcli {{ action }} route {{ route }} next-hop {{ vpn_a_ip_overlay }} as-path [ ] local-preference 150
env exabgp.api.pipename=exabgp{{ index }} exabgpcli {{ action }} route {{ route }} next-hop {{ vpn_a_ip_overlay }} as-path [ ] local-preference 150

"""
template_exabgpenv = """

[exabgp.api]
pipename = 'exabgp{{ index }}'
...
configuration = true
daemon = true
destination = '/var/log/exabgp{{ index }}.log'
enable = true
"""
...
#################################################################################
print("Cleanup Linux IPSEC config file ...")
files = glob.glob('/etc/ipsec.d/tunnel*.conf')
for file in files:
    os.remove(file)
...
#################################################################################
print("Cleanup Linux ExaBGP config file ...")
files = glob.glob('/etc/exabgp/exabgp*.*')
for file in files:
    os.remove(file)
...
while index <= index_end_page:
  ...
  filename2 = "/etc/exabgp/exabgp" + str(index) + ".env"
  f2 = open(filename2, "w")
  tm2 = Template(template_exabgpenv)
  msg2 = tm2.render(index=index)
  f2.write(msg2)
  f2.close()
  ...
  print("Startup exabgp for batch " + str(index) + " with: env exabgp.api.pipename=exabgp" + str(index) + " exabgp " + filename + " > /var/log/exabgp" + str(index) + ".log")
  f.close()
  index += 1
...
while index <= index_end_page:
  ...
  while index2 <= (index*page):
    ...
    route_a_next = str(ipaddress.IPv4Address(route_a_int + (index2 * 256))) + "/24"
    route_b_next = str(ipaddress.IPv4Address(route_b_int + (index2 * 256))) + "/24"
    vpn_a_ip_overlay = str(ipaddress.IPv4Address(ip_int_overlay + index2))

    tm = Template(template_exabgpcli)
    msg = tm.render(index=index,action="announce",vpn_a_ip_overlay=vpn_a_ip_overlay, route=route_a_next)
    f.write(msg)
    tm = Template(template_exabgpcli)
    msg = tm.render(index=index,action="withdraw",vpn_a_ip_overlay=vpn_a_ip_overlay, route=route_a_next)
    f2.write(msg)
    tm = Template(template_exabgpcli)
    msg = tm.render(index=index,action="announce",vpn_a_ip_overlay=vpn_a_ip_overlay, route=route_b_next)
    f3.write(msg)
    tm = Template(template_exabgpcli)
    msg = tm.render(index=index,action="withdraw",vpn_a_ip_overlay=vpn_a_ip_overlay, route=route_b_next)
    f4.write(msg)
