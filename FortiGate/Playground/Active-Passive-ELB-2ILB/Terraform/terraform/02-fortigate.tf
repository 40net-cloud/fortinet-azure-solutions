...
resource "azurerm_lb" "ilb" {
  ...
  frontend_ip_configuration {
    name                          = "${var.PREFIX}-ILB-PIP2"
    subnet_id                     = azurerm_subnet.subnet7.id
    private_ip_address            = var.lb_internal_ipaddress_b
    private_ip_address_allocation = "Static"
  }
}
...
resource "azurerm_lb_backend_address_pool" "ilbbackend2" {
  loadbalancer_id = azurerm_lb.ilb.id
  name            = "BackEndPool2"
}
...
resource "azurerm_lb_rule" "lb_haports_rule2" {
  resource_group_name            = azurerm_resource_group.resourcegroup.name
  loadbalancer_id                = azurerm_lb.ilb.id
  name                           = "lb_haports_rule_dmz"
  protocol                       = "All"
  frontend_port                  = 0
  backend_port                   = 0
  frontend_ip_configuration_name = "${var.PREFIX}-ILB-PIP2"
  probe_id                       = azurerm_lb_probe.ilbprobe.id
  backend_address_pool_ids       = [azurerm_lb_backend_address_pool.ilbbackend2.id]
}
...
resource "azurerm_network_interface" "fgtaifcdmz" {
  name                          = "${var.PREFIX}-A-VM-FGT-IFC-DMZ"
  location                      = azurerm_resource_group.resourcegroup.location
  resource_group_name           = azurerm_resource_group.resourcegroup.name
  enable_ip_forwarding          = true
  enable_accelerated_networking = var.FGT_ACCELERATED_NETWORKING
...
resource "azurerm_network_interface_security_group_association" "fgtaifcdmznsg" {
  network_interface_id      = azurerm_network_interface.fgtaifcdmz.id
  network_security_group_id = azurerm_network_security_group.fgtnsg.id
}

resource "azurerm_network_interface_backend_address_pool_association" "fgtaifcdmz2ilbbackendpool2" {
  network_interface_id    = azurerm_network_interface.fgtaifcdmz.id
  ip_configuration_name   = "interface1"
  backend_address_pool_id = azurerm_lb_backend_address_pool.ilbbackend2.id
}
...
resource "azurerm_virtual_machine" "fgtavm" {
  ...
  location                     = azurerm_resource_group.resourcegroup.location
  resource_group_name          = azurerm_resource_group.resourcegroup.name
  network_interface_ids        = [azurerm_network_interface.fgtaifcext.id, azurerm_network_interface.fgtaifcint.id, azurerm_network_interface.fgtaifchasync.id, azurerm_network_interface.fgtaifcmgmt.id,  azurerm_network_interface.fgtaifcdmz.id]
...
data "template_file" "fgt_a_custom_data" {
  ...
  vars = {
    ...
    fgt_dmz_ipaddr     = var.fgt_ipaddress_a["7"]
    fgt_dmz_mask       = var.subnetmask["7"]
    fgt_dmz_gw         = var.gateway_ipaddress["7"]
...
resource "azurerm_network_interface" "fgtbifcdmz" {
  name                          = "${var.PREFIX}-B-VM-FGT-IFC-DMZ"
  location                      = azurerm_resource_group.resourcegroup.location
  resource_group_name           = azurerm_resource_group.resourcegroup.name
  enable_ip_forwarding          = true
  enable_accelerated_networking = var.FGT_ACCELERATED_NETWORKING
...
resource "azurerm_network_interface_security_group_association" "fgtbifcdmznsg" {
  network_interface_id      = azurerm_network_interface.fgtbifcdmz.id
  network_security_group_id = azurerm_network_security_group.fgtnsg.id
}

resource "azurerm_network_interface_backend_address_pool_association" "fgtbifcdmz2ilbbackendpool2" {
  network_interface_id    = azurerm_network_interface.fgtbifcdmz.id
  ip_configuration_name   = "interface1"
  backend_address_pool_id = azurerm_lb_backend_address_pool.ilbbackend2.id
}
...
resource "azurerm_virtual_machine" "fgtbvm" {
  ...
  location                     = azurerm_resource_group.resourcegroup.location
  resource_group_name          = azurerm_resource_group.resourcegroup.name
  network_interface_ids        = [azurerm_network_interface.fgtbifcext.id, azurerm_network_interface.fgtbifcint.id, azurerm_network_interface.fgtbifchasync.id, azurerm_network_interface.fgtbifcmgmt.id, azurerm_network_interface.fgtbifcdmz.id]
...
data "template_file" "fgt_b_custom_data" {
  ...
  vars = {
    ...
    fgt_mgmt_mask       = var.subnetmask["4"]
    fgt_mgmt_gw         = var.gateway_ipaddress["4"]
    fgt_dmz_ipaddr      = var.fgt_ipaddress_b["7"]
    fgt_dmz_mask        = var.subnetmask["7"]
    fgt_dmz_gw          = var.gateway_ipaddress["7"]
