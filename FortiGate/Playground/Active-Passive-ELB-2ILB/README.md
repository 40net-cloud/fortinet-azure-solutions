...
This design adds a second internal interface to separate for example ExpressRoute, use zones or VDOM. Deployment is similar to the basic building block Active-Passive with External and Internal Azure Load Balancer found [here](../../Active-Passive-ELB-ILB/README.md). The main difference is that we added an extra network interface. A Terraform version is available [here](Terraform/)
