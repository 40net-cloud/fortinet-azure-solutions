...
# Fortigate Active/Passive cluster with Application Gateway + Internal Load Balancer
#
##############################################################################################################
...
az deployment group create --resource-group "$rg" \
                           --template-file azuredeploy.json \
                           --parameters adminUsername="$USERNAME" adminPassword=$PASSWORD fortigateNamePrefix=$prefix --debug
result=$?
if [[ $result != 0 ]];
then
